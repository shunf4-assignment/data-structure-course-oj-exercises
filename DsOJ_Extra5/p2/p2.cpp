...
bool do_operation(LinkStack<char> &oprtStack, LinkStack<int> &opndStack)
{
	char oprt_chr = '.';
	int opndA, opndB;

	if (oprtStack.pop(oprt_chr) && opndStack.pop(opndB) && opndStack.pop(opndA))
	{
		switch (oprt_chr)
		{
			case '+':
				opndStack.push(opndA + opndB);
				break;
			case '-':
				opndStack.push(opndA - opndB);
				break;
...
int main()
{
...
#endif
	int opnd;
	char oprt_chr = '\0', test_chr;
	bool error = false;
	bool even = true;
	LinkStack<char> oprtStack;
	LinkStack<int> opndStack;
	...
	while (!cin.eof())
	{
		...
		else
		{
			...
			else if (oprt_chr == '+' || oprt_chr == '-')
			{
				if (oprtStack.peek(test_chr) && (test_chr == '+' || test_chr == '-' || test_chr == '*' || test_chr == '/'))
				{
					if (!do_operation(oprtStack, opndStack))
					{
						error = true;
						break;
					}
				}
				oprtStack.push(oprt_chr);
			}
			else if (oprt_chr == '*' || oprt_chr == '/')
			{
				if (oprtStack.peek(test_chr) && (test_chr == '*' || test_chr == '/'))
				{
					if (!do_operation(oprtStack, opndStack))
					{
						error = true;
			...
			{
				while (oprtStack.peek(test_chr) && test_chr != '(')
				{
					if (!do_operation(oprtStack, opndStack))
					{
						error = true;
	...
	if (!error)
	{
		...
		if (oprtStack.length != 0 || opndStack.length != 1)
			error = true;
	}
	if (!error)
	{
		int test_int;
		opndStack.peek(test_int);
		cout << test_int << endl;
	}
	else
	{
