...
template <typename VertexElem, typename EdgeWeight>
size_t inputGraph_Dijkstra (std::istream & in, Graph_List<VertexElem, EdgeWeight> &ga)
{
	if (ga.v)
		delete[]ga.v;
	...
	ga.setInfinity(INT_MAX);

	size_t vCnt, eCnt, firstVertex;
	in >> vCnt >> eCnt >> firstVertex;
	firstVertex--;
	ga.vertexNum = vCnt;
...
int main()
{
	...
	if (1)
	{
		...
		size_t firstVertex = inputGraph_Dijkstra(std::cin, g);
		g.output_dijkstra(std::cout, firstVertex);
	}
